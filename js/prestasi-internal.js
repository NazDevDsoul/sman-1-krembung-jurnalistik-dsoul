document.addEventListener("DOMContentLoaded", () => {
  const container = document.getElementById("prestasiInternalContainer");
  const loading = document.getElementById("loading");
  const filterButtons = document.querySelectorAll(".filter button");
  const heroSlider = document.getElementById("prestasiInternalSlider");
  const hamburger = document.getElementById("hamburger-menu");
  const navMenu = document.querySelector(".navbar-nav");

  const CACHE_KEY = "prestasi_internal_cache_v1";
  ...
  function showLoading(text = "Memuat prestasi internal...") {
    if (!loading) return;
  ...
  function renderList(data) {
    ...
    if (!data.length) {
      container.innerHTML = "<div style='text-align:center;padding:2rem;color:#555;'>Belum ada prestasi internal yang tersedia.</div>";
      return;
    }
  ...
  fetchJson("prestasi-internal")
    ...
    .catch(err => {
      console.error("Gagal load prestasi internal:", err);
      if (!cached) showLoading("Gagal memuat prestasi internal.");
    });
});